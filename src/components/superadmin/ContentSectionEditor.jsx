...
const inputClass =
  'mt-0.5 w-full rounded-lg border border-line bg-card px-2 py-1.5 text-sm text-heading focus:border-line-strong focus:outline-none'
    ...
    <div className="rounded-2xl border border-line bg-card p-5">
        ...
        <div className="flex items-center gap-4">
          ...
          <label className="flex items-center gap-2 text-sm text-body">
            ...
            <input
              ...
              className="w-16 rounded-lg border border-line bg-card px-2 py-1 text-sm text-heading focus:border-line-strong focus:outline-none"
            />
          </label>
        </div>
      </div>
        ...
        <>
          <div className="mt-4 space-y-3">
            {items.map((item, index) => (
              <div
                key={index}
                className="flex items-start gap-3 rounded-lg border border-line bg-card-strong p-3"
              >
                ...
                <div className="flex shrink-0 flex-col items-center gap-1 pt-4">
                  <button
                    ...
                    disabled={index === 0}
                    title="Move up"
                    className="cursor-pointer text-xs text-faint hover:text-heading disabled:cursor-not-allowed disabled:opacity-30"
                  >
                    ▲
                  </button>
                  <button
          ...
          <div className="mt-4">
            <button
              type="button"
              onClick={addItem}
              className="cursor-pointer rounded-lg border border-line px-3 py-1.5 text-sm font-medium text-body transition-colors hover:bg-card-strong hover:text-heading"
            >
              + Add item
            </button>
          </div>
      ...
      <div className="mt-4 flex flex-wrap items-center gap-3">
        <button
          ...
          onClick={handleSave}
          disabled={saving}
          className="cursor-pointer rounded-lg bg-indigo-600 px-4 py-1.5 text-sm font-medium text-white transition-colors hover:bg-indigo-500 disabled:cursor-not-allowed disabled:opacity-50"
        >
